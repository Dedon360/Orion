<?php
	...
	if(isset($_POST['submit'])){
		...
		if($product_name == ""){
			...
			header('location: ../veiw/orderupload.php?error='.$error);
			return false;
		}else{
			...
			if(in_array($fileactualext, $allow)){
				if($filesize < 800000){
					...
					if(move_uploaded_file($fileTmp, $filedestination)){
						$sql = "INSERT INTO order_tb(product_name,product_price,product_tags,product_img) 
                        VALUES('$product_name', '$product_price', '$product_tags', '$product_img')";
						$result = mysqli_query($con, $sql);
						if($result){
							$success = "details uploaded";
							header('location: ../veiw/orderupload.php?success='.$success);
							return false;
						}else{
